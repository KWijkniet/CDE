...
__publicField(Vector2, "dot", (v1, v2) => {
  return v1.x * v2.x + v1.y * v2.y;
});
__publicField(Vector2, "abs", (v) => {
  return new _Vector2(Math.abs(v.x), Math.abs(v.y));
});
...
_diff = new WeakMap();
_raycast = new WeakSet();
raycast_fn = function(shapes, from, dir, dist) {
...
class Tile {
  constructor(vertices = [], buffer = null, isDummy = false) {
    ...
    this.width = 0;
    this.height = 0;
    this.pos = __privateGet(this, _vertices2)[0];
    ...
    if (isDummy) {
      __privateSet(this, _color, Settings.type == "Zwart" ? Settings.dummyZwartBackground : Settings.dummyTerracottaBackground);
    } else {
      __privateSet(this, _color, Settings.type == "Zwart" ? Settings.tileZwartBackground : Settings.tileTerracottaBackground);
    }
    ...
    if (buffer == null) {
      __privateSet(this, _buffer4, createGraphics(this.width, this.height));
    }
  ...
  getWidth() {
    return __privateGet(this, _vertices2)[1].x - __privateGet(this, _vertices2)[0].x;
  }
  getHeight() {
    return __privateGet(this, _vertices2)[2].y - __privateGet(this, _vertices2)[1].y;
  }
}
...
generate_fn3 = function() {
  ...
  var rgba = __privateGet(this, _color).rgba();
...
class GeneratorTool {
  constructor() {
    ...
    __privateAdd(this, _createTile);
    __privateAdd(this, _raycast2);
    __publicField(this, "isEnabled", false);
    __publicField(this, "canAdd", true);
    __publicField(this, "canDelete", true);
    __publicField(this, "canInsert", true);
...
createOutset_fn = function(shape) {
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    {
      __privateGet(this, _buffer5).fill(0, 255, 0);
      __privateGet(this, _buffer5).circle(posP.x, posP.y, 10);
      __privateGet(this, _buffer5).circle(posN.x, posN.y, 10);
      __privateGet(this, _buffer5).circle(pos.x, pos.y, 10);
    }
    if (Collision.polygonCircle(shape.getVertices(), pos.x, pos.y, 5)) {
      ...
      {
        __privateGet(this, _buffer5).fill(0, 0, 255);
        __privateGet(this, _buffer5).circle(posP.x, posP.y, 10);
        __privateGet(this, _buffer5).circle(posN.x, posN.y, 10);
        __privateGet(this, _buffer5).circle(pos.x, pos.y, 10);
      }
...
generateTiles_fn = function(inset, outsets) {
  ...
  var syncedPlaceTile = async (x, y) => new Promise((resolve) => {
    ...
    setTimeout(() => {
      var _a;
      var horizontalTop = __privateMethod(_a = self, _raycast2, raycast_fn2).call(_a, [inset], pos, Vector2.left(), isFirstTile ? firstTileSize.x : tileSize.x);
      if (!horizontalTop) {
      ...
      __privateGet(self, _buffer5).line(pos.x, pos.y, horizontalTop.x, horizontalTop.y);
      setTimeout(() => {
        var _a2;
        var verticalRight = __privateMethod(_a2 = self, _raycast2, raycast_fn2).call(_a2, [inset], horizontalTop, Vector2.down(), tileSize.y);
        if (!verticalRight) {
        ...
        __privateGet(self, _buffer5).line(horizontalTop.x, horizontalTop.y, verticalRight.x, verticalRight.y);
        setTimeout(() => {
          var _a3;
          var maxDist = isFirstTile ? tileSize.x : Vector2.distance(pos, horizontalTop);
          var horizontalBottom = __privateMethod(_a3 = self, _raycast2, raycast_fn2).call(_a3, [inset], verticalRight, Vector2.right(), maxDist);
          if (!horizontalBottom) {
          ...
          __privateGet(self, _buffer5).line(verticalRight.x, verticalRight.y, horizontalBottom.x, horizontalBottom.y);
          setTimeout(() => {
            var _a4, _b;
            var verticalLeft = __privateMethod(_a4 = self, _raycast2, raycast_fn2).call(_a4, [inset], verticalRight, Vector2.right(), tileSize.x);
            if (!verticalLeft) {
            ...
            __privateGet(self, _buffer5).line(verticalLeft.x, verticalLeft.y, pos.x, pos.y);
            resolve(__privateMethod(_b = self, _createTile, createTile_fn).call(_b, [pos, horizontalTop, verticalRight, horizontalBottom, verticalLeft], isDummy));
          }, delay);
        }, delay);
      }, delay);
    }, delay);
  ...
  var syncedLoop = async (x, y) => {
    ...
    if (!this.IsInside(insetPoints, x, y)) {
      ...
      var horizontal = __privateMethod(_a = self, _raycast2, raycast_fn2).call(_a, [inset], pos, Vector2.left(), boundingBox.w);
      if (horizontal) {
        if (!this.IsInside(insetPoints, horizontal.x, horizontal.y)) {
          pos = horizontal.getCopy().add(Vector2.down().multiply(new Vector2(10, 10)));
          var vertical = __privateMethod(_b = self, _raycast2, raycast_fn2).call(_b, [inset], pos, Vector2.down(), boundingBox.h);
          if (vertical) {
    ...
    if (resultPos) {
      var tmp = null;
      await syncedPlaceTile(resultPos.x, resultPos.y).then((tile2) => {
        tmp = tile2;
      });
      if (tmp != null) {
        await __privateGet(_c = self, _sleep).call(_c, 500);
        isFirstTile = false;
      ...
      await __privateGet(_d = self, _sleep).call(_d, 100);
      syncedLoop(x, y + 1);
    }
  };
  syncedLoop(boundingBox.x, boundingBox.y);
};
_createTile = new WeakSet();
createTile_fn = function(vertices, isDummy) {
  return new Tile(vertices, __privateGet(this, _buffer5), isDummy);
};
_raycast2 = new WeakSet();
raycast_fn2 = function(shapes, from, dir, dist, ignoreSelf = true) {
  var collisionClosest = null;
  var collisionDist = 99999999999;
  var end = from.getCopy().remove(new Vector2(dir.x, dir.y).multiply(new Vector2(dist, dist)));
  for (let i = 0; i < shapes.length; i++) {
...
export {
  ...
  Cursor$1 as Cursor,
  DrawingTool,
  EventSystem$1 as EventSystem,
  GeneratorTool,
  Grid,
  HistoryTool$1 as HistoryTool,
  LineSelectorTool,
  Renderer$1 as Renderer,
  SelectorTool,
