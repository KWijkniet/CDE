...
var _canvas, _activeTool, _events, _mousedown, _mousemoved, _lastPos, _diff, _event, event_fn, _checkBounds, checkBounds_fn, _buffer, _vertices, _shapebuffer, _textBuffer, _pos, _color, _generate, _generateUniqSerial, _shapes, _buffer2, _points, _selectedPointIndex, _onPlace, onPlace_fn, _onDrag, onDrag_fn, _generate2, generate_fn;
class Collision {
...
class Shape {
  ...
  update() {
    image(__privateGet(this, _shapebuffer), 0, 0);
  }
  updateText() {
    image(__privateGet(this, _textBuffer), 0, 0);
  }
}
  ...
  for (let i = 0; i < __privateGet(this, _points).length; i++) {
    ...
    if (dist <= Settings.cursorSize) {
      if (i != 0) {
        ...
        hasFound = true;
        Renderer.instance.add(new Shape(__privateGet(this, _points), new Color("--shape-allowed")));
        __privateSet(this, _points, []);
